...
class VisitPlanAdd extends StatefulWidget {
  static String routeName = "/visitplanadd";

  DateTime planDate;

  VisitPlanAdd({this.planDate});
...
class _VisitPlanAddState extends State<VisitPlanAdd> {
  ...
  @override
  void initState() {
    VisitProvider().getOutlVisitPlans(widget.planDate.toString()).then((value) {
      if (!value.success) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: DefaultControl.headerText(
            headText:
                'Visit Plan' + widget.planDate.toString().substring(0, 11)),
        actions: [
          IconButton(
            icon: Icon(
              Icons.refresh,
  ...
  Widget CardSection(Result nodesForDisplay) {
    bool _isChecked = nodesForDisplay.OutlSelected == 1 ? true : false;
        ...
        children: [
          GestureDetector(
              ...
              child: Column(
                ...
                children: [
                  ListTile(
                    title: Text("${nodesForDisplay.OutletName}",
                        style: TextStyle(
                            color: Colors.black,
                            fontSize: 17.0,
                            fontWeight: FontWeight.bold)),
                    subtitle: Text(
                      "${nodesForDisplay.OutletID}",
                    ...
                    trailing: Checkbox(
                        ...
                        onChanged: (value) {
                          setState(() {
                            _isChecked = value;
                            if (value) {
                              nodesForDisplay.OutlSelected = 1;
                              detailClick(nodesForDisplay);
                            } else {
                              nodesForDisplay.OutlSelected = 0;
                              detailClick(nodesForDisplay);
                            }
                          });
                        }),
